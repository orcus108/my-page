...
Use normal capitalization throughout site copy, except for the nav bar. Don't use em-dashes anywhere in the site.
...
deployed on Vercel at https://vedantmisra.dev. `vercel.json` sets `buildCommand: npm run build` and `outputDirectory: site`, so the live site is the main design generated by `scripts/site.mjs`. `npm run build` regenerates both the legacy root site and `site/`; only `site/` is served. the local content dashboard (`npm run dashboard`) writes to `content/`, rebuilds `site/`, and previews it. `legacy-site/` is a static snapshot of the older deployed site for rollback.
